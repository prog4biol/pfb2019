...
# Finally determine which subset of codons produced the longest peptide for each sequence record. Print this to a fourth file in FASTA format (Python_08.orf-longest.nt).
...
table_reversed = {aa:codon for codon, aa in translation_table.items()}
...
f_longest_orf = open('output/Python_08.orf-longest.nt', 'w')
...
for seq_id in seqs:
    ...
    for i in range(0, 3):
      ...
      ## keep our frame strings for writing out 'biologically relevant' names 
    ...
    ## I got really tired of this problem
    ## I decided to just reverse the dictionary and pull out my CDS based on my AA sequence
    ## You can see the code to reverse the dicionary directly under the first translation table
    longest_orf_list = [table_reversed.get(x) for x in longest_aa]
    longest_orf = "".join(longest_orf_list)
    ...
    ## the longest amino acid
    f_longest_aa.write(">" + seq_id + "\n") 
    f_longest_aa.write(longest_aa + "\n")

    ## the longest orf
    f_longest_orf.write(">" + seq_id + "\n") 
    f_longest_orf.write(longest_orf + "\n")
